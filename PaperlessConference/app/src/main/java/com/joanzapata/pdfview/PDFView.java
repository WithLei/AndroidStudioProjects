...
public class PDFView extends SurfaceView {
    ...
    /** Rendered parts go to the cache manager 提供的部分转到缓存管理器*/
    private CacheManager cacheManager;

    /** Animation manager manage all offset and zoom animation 动画管理器管理所有偏移和缩放动画。*/
    private AnimationManager animationManager;

    /** Drag manager manage all touch events 拖动管理器管理所有触摸事件*/
    private DragPinchManager dragPinchManager;

    /**
     * The pages the user want to display in order用户要按顺序显示的页面
     * (ex: 0, 2, 2, 8, 8, 1, 1, 1)
     */
    private int[] originalUserPages;

    /**
     * The same pages but with a filter to avoid repetition 相同的页面，但有一个过滤器，以避免重复
     * (ex: 0, 2, 8, 1)
     */
    ...
    /** Number of pages in the loaded PDF document 加载的PDF文档中的页面数。*/
    private int documentPageCount;

    /** The index of the current sequence 当前序列的索引。*/
    private int currentPage;

    /** The index of the current sequence 当前过滤序列的索引。*/
    private int currentFilteredPage;

    /** The actual width and height of the pages in the PDF document PDF文档中页面的实际宽度和高度。*/
    private int pageWidth, pageHeight;

    /** The optimal width and height of the pages to fit the component size 页面的最佳宽度和高度适合组件大小。*/
     ...
     * 如果你把所有的页面都画在最优宽度上，考虑到缩放级别，当前偏移量为。画面左侧边框的位置。
     */
    ...
    /** The zoom level, always 缩放级别,总是>= 1 */
    private float zoom = 1f;

    /** Coordinates of the left mask on the screen 在屏幕上的左遮罩的坐标。*/
    private RectF leftMask;

    /** Coordinates of the right mask on the screen 在屏幕上的右遮罩的坐标*/
    private RectF rightMask;

    /** True if the PDFView has been recycled pdfview是否已回收*/
    private boolean recycled = true;

    /** Current state of the view 视图当前状态*/
    private State state = State.DEFAULT;

    /** The VuDroid DecodeService used for decoding PDF and pages VuDroid DecodeService用于解码PDF和页面。*/
    private DecodeService decodeService;

    /** Async task used during the loading phase to decode a PDF document 在加载阶段用于解码PDF文档的异步任务。*/
    private DecodingAsyncTask decodingAsyncTask;

    /** Async task always playing in the background and proceeding rendering tasks 异步任务总是在后台运行，并进行呈现任务。*/
    private RenderingAsyncTask renderingAsyncTask;

    /** Call back object to call when the PDF is loaded 在加载PDF时调用返回对象*/
    private OnLoadCompleteListener onLoadCompleteListener;

    /** Call back object to call when the page has changed 当页面发生更改时，调用back对象来调用*/
    private OnPageChangeListener onPageChangeListener;

    /** Call back object to call when the above layer is to drawn 在绘制上述图层时，调用back对象调用。*/
    private OnDrawListener onDrawListener;

    /** Paint object for drawing 绘画对象的绘画*/
    private Paint paint;

    /** Paint object for drawing mask 绘画对象的绘画面具*/
    private Paint maskPaint;

    /** Paint object for drawing debug stuff 绘图对象，用于绘图调试内容*/
    private Paint debugPaint;

    /** Paint object for minimap background 用于小地图背景的绘画对象。*/
    private Paint paintMinimapBack;

    private Paint paintMinimapFront;

    /** True if should draw map on the top right corner 如果应该在右上角绘制地图。*/
    private boolean miniMapRequired;

    /** Bounds of the minimap 小地图上的边界*/
    private RectF minimapBounds;

    /** Bounds of the minimap 小地图屏幕上的边界*/
    ...
    /** True if should scroll through pages vertically instead of horizontally 如果应该垂直滚动页面而不是水平页面。*/
    private boolean swipeVertical = false;

    /** Construct the initial view 构建初始视图*/
